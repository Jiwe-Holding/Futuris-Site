...
const LoadingScreen: React.FC = () => {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-white flex items-center justify-center transition-all duration-500 ${
      progress >= 100 ? 'opacity-0 pointer-events-none' : 'opacity-100'
    }`}>
      {/* Background Animation */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-gray-200/40 rounded-full blur-3xl animate-pulse delay-1000"></div>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 bg-blue-600 rounded-full animate-float opacity-20"
            style={{
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 text-lg">Excellence in Market Research</p>
        </div>
